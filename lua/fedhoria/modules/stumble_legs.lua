...
local stumble_time 	= CreateConVar("fedhoria_stumble_time", 0, bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED))
local grabright_chance 	= CreateConVar("fedhoria_grabright_chance", 0.5, bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED))
local grableft_chance 	= CreateConVar("fedhoria_grableft_chance", 0.5, bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED))
local grab_time 	= CreateConVar("fedhoria_woundgrab_time", 6.5, bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED))
...
local hand_offset = Vector(0, 0, 0)
...
function MODULE:Init(phys_bone, lpos)
	...
	if phys_bone then
		...
		if (IsValid(phys) and phys_bone != self.phys_bone_lhand and phys_bone != self.phys_bone_rhand) then
			...
			if (IsValid(phys_lhand) and math.random() < grableft_chance:GetFloat()) then
				local const = ents.Create("phys_spring")
				const:SetPos(phys_lhand:LocalToWorld(hand_offset))
				const:SetKeyValue("springaxis", str_axis)
				const:SetKeyValue("constant", 865)
				const:SetKeyValue("damping", 0)
				const:SetKeyValue("relativedamping", 0)
				const:SetPhysConstraintObjects(phys_lhand, phys)
				const:Spawn()
				const:Activate()
				const:Fire("SetSpringLength", -1, 0.05)
			...
			if (IsValid(phys_rhand) and math.random() < grabright_chance:GetFloat()) then
				local const = ents.Create("phys_spring")
