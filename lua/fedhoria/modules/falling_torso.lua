...
local die_time = CreateConVar("fedhoria_dietime", 10, bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED))
...
function MODULE:PhysicsSimulate(phys, dt)
	...
	if (phys_bone == 0) then
		...
		if (target:WaterLevel() > 0) then
			self:ResetSequence(self:LookupSequence("Moveonfire"))
			self.StartDie = self.StartDie or CurTime()	
			self:SetPlaybackRate(1.45 * f)
		else
			self:ResetSequence(self:LookupSequence("idleonfire"))
